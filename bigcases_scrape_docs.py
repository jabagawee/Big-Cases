...
COSTS = [
    re.compile(
        'Cost: </FONT></TH><TD ALIGN=LEFT><FONT SIZE=-1 COLOR=DARKBLUE>(.*?)<',
        re.IGNORECASE),
    re.compile(
        'Cost: </font></th><td align="LEFT"><font size="-1" color="DARKBLUE">(.*?)<',
        re.IGNORECASE),
...
def handleLogin():
    ...
    br.find_element_by_name('login:clientCode').send_keys(
        settings.pacer_client)
...
def getDocument(case, url):
    ...
    MULTIPATTERN = re.compile(
        '<td colspan=\"?2\"?><b>Document Number:</b> <a.*?>(\d+)<',
        re.IGNORECASE)
    ...
    if price <= settings.pacer_max_price and price is not None:
        ...
        if len(files) >= 2:
            newfn = settings.file_archive_path + '/document-' + str(
                pid) + '.pdf'
            print '   - now: ' + newfn
